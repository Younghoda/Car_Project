...
imgTopHat = cv2.morphologyEx(imgray, cv2.MORPH_TOPHAT, structuringElement)
imgBlackHat = cv2.morphologyEx(imgray, cv2.MORPH_BLACKHAT, structuringElement)

imgGrayscalePlusTopHat = cv2.add(imgray, imgTopHat)
gray = cv2.subtract(imgGrayscalePlusTopHat, imgBlackHat)

# 3. 가우시안블러 -> thresholding
...
cv2.imshow('5_1', temp_result)

### 5-2. Select Candidates by Char Size
MIN_AREA = 80
...
cv2.imshow('5_2', temp_result)

### 5-3. Select Candidates by Arrangement of Contours
...
cv2.imshow('5_3', temp_result)

### 5_4. Rotate Plate Images
...
for i, plate_img in enumerate(plate_imgs):
    ...
    pattern = r"\d{2,3}[가-힣]{1}\d{4}"  # 정규 표현식 패턴
